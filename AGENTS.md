...
| `tavily-mcp_tavily_search`       | Recent risk events, macro shocks context (3-month lookback) | Phase 11                 |
...
### Automated Script (Data Fetching Utility)

A Python script (`risk_research.py`) acts as a dedicated data fetcher. It runs prior to the LLM's analytical phases to avoid excessive, rate-limited MCP calls to the SEC. It dumps the *raw text* of the 10-K, proxy, financials, and market data into a local directory so the LLM can instantly read and analyze it.

```bash
# Fetch all raw data for a single company into ./dist/{TICKER}/raw/
python risk_research.py JPM

# Custom output directory
python risk_research.py JPM --output-dir ./dist/JPM/raw
```

Requirements: `pip install edgartools yfinance pandas`
Set SEC identity: `set EDGAR_IDENTITY="Your Name your@email.com"`

Once the script completes, the LLM reads the files in the output directory (e.g., `item_1A_risk_factors.txt`, `financial_statements.txt`, `relevant_notes.txt`, `metadata.json`) to perform the parsing, thematic categorization, and generation of the final 13-phase deliverables.

**CRITICAL INSTRUCTION ON MCP USAGE:** 
The LLM agent must **primarily rely on the raw data files** generated by the script. The explicit MCP calls listed under the `### Steps` for each phase below are provided as **optional fallbacks or drill-down methods**. Only execute those MCP tools if the script's raw data is insufficient, missing, or if a deeper investigation (like full-text searching across historical 8-Ks) is required.

### Dynamic MCP Fetches (LLM Investigatory Work)

While the script handles bulk static document downloads, the LLM agent is responsible for **dynamic, context-dependent, and investigatory** queries using MCP tools. Examples include:

1. **Event-Driven Full-Text Searches (Phase 9 & 11)**:
   - *Tools:* `edgartools_edgar_text_search`, `edgartools_edgar_monitor`
   - *Use Case:* If you read about a vague "recent third-party vendor breach" in the 10-K, dynamically query `vendor data breach` within recent 8-Ks to find out what happened.
2. **Macro Shock Context & Web Search (Phase 11)**:
   - *Tools:* `tavily-mcp_tavily_search`
   - *Use Case:* Dynamically search the live web for recent external shocks impacting the company (e.g., commercial real estate stress, AI disruption) that may not yet be reflected in SEC filings.
3. **Peer Comparisons (Phase 6)**:
   - *Tools:* `edgartools_edgar_compare`
   - *Use Case:* If you calculate a 33% increase in Provision for Credit Losses, compare it with peers (e.g., BAC, WFC) to see if it's an industry-wide shock or company-specific.
4. **Deep Financial Trends (Phase 6)**:
   - *Tools:* `edgartools_edgar_trends`
   - *Use Case:* If a specific metric like `allowance for credit losses` shows an alarming trend in the 3-year data, pull 10 years of data to map it against historical baselines.
5. **Insider Trading Verification (Phase 10)**:
   - *Tools:* `edgartools_edgar_ownership`
   - *Use Case:* If you read about a massive pending lawsuit, check if insiders dumped stock before the filing.
6. **Proxy Governance Drill-Down (Phase 5)**:
   - *Tools:* `edgartools_edgar_read` (sections=["governance"])
   - *Use Case:* If the script indicates "Risk committee not found in Proxy", fall back to exploring the full proxy text or use `edgartools_edgar_read` to extract the governance section and look for alternatives like "Audit and Compliance Committee".
...
```
1. tavily-mcp_tavily_search(query="[Company Name] (lawsuit OR penalty OR cyber breach OR downgrade OR macro shock)", search_depth="advanced", max_results=15)
   → Set the time_range to "month" or calculate start_date and end_date to capture the last 3 months (90 days).
...
| Output                                      | Source                                                                                       |
| ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| Recent risk events identified               | `tavily_search` (Tavily), 8-K filings                                                      |
| Financial impact (if quantified in filings) | 10-Q, 8-K, external news sources                                                             |
...
> **Caption:** Recent risk event impact requires Tavily search analysis over the last 3 months and 8-K monitoring. Must cite source and date.
