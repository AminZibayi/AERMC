...
From Phases 11 and 12, list 3-5 top macro/emerging risk themes (e.g., "geopolitical escalation", "AI-driven cyber attacks", "Basel III endgame capital rule revision", "Commercial real estate systemic stress").

### 13.2 Map Each Theme to Firm-Specific Exposures

For each macro theme:

1. Identify the firm's direct exposure (e.g., "X bank has $224.9B CRE exposure per Note 4" [^n])
...
- Write a cross-scenario synthesis CSV to `./dist/<TICKER>/artifacts/scenario_synthesis.csv` and reference the artifact path in the report.
